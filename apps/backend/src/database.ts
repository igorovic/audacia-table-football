...
export async function getPlayerStats(playerId: number) {
  ...
  playerGames.forEach((game) => {
    ...
    const opponentGoals =
      game.game.participants.find((p) => p.playerId !== playerId)?.goals || 0
...
export async function getRankedPlayers() {
  // only select players who have participated in a game
  const players = await prisma.gameParticipant.findMany({
    distinct: ['playerId'],
    include: { player: true },
  })
  ...
  const rankedPlayers = await Promise.all(
    players.map(async (player) => ({
      ...player.player,
      ...(await getPlayerStats(player.playerId)),
    })),
  )
